...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "UPDATE posts SET 
              title='$title', slug='$slug', category_id=$primary_category_id, additional_categories='$additional_categories', writer_id=$writer_id, description='$description', 
              read_online_url='$read_online_url', download_url='$download_url', file_size='$file_size', file_type='$file_type', pages=$pages, status='$status',
              meta_title='$meta_title', meta_description='$meta_description', updated_at = CURRENT_TIMESTAMP $image_query 
              WHERE id=$id";
    
    if ($conn->query($query) === TRUE) {
        clear_page_cache();
        $msg = "<div class='alert alert-success'><i class='fa-solid fa-check-circle'></i> Post updated successfully!</div>";
    } else {
        if($conn->errno == 1062) {
...
<head>
    ...
    <style>
        ...
        .btn-trash { background: #ffc107; color: black; border: none; font-weight: 600; padding: 10px 20px; border-radius:0; width: 100%; transition: 0.3s;}
        .btn-trash:hover { background: #e0a800; color: black;}
    ...
    <div class="main-content">
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3 class="fw-bold mb-0 text-white">Edit Post</h3>
                <a href="..<?php echo get_post_link($row['id'], $row['slug']); ?>" target="_blank" class="btn btn-outline-secondary btn-sm"><i class="fa-solid fa-eye me-2"></i> View Live</a>
            </div>
            ...
            <div class="row">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" name="meta_title" class="form-control" value="<?php echo htmlspecialchars($row['meta_title'] ?? ''); ?>" placeholder="SEO Title (defaults to post title if empty)">
                            </div>
                            <div class="mb-3">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <button type="submit" name="status" value="active" class="btn-publish"><i class="fa-solid fa-arrows-rotate me-2"></i> Update</button>
                        ...
                        <div class="card-body">
                            ...
                            <select name="writer_id" id="writerSelect" class="form-select mb-2" required>
                                ...
                                while($w = $writers->fetch_assoc()){
                                    $sel = ($row['writer_id'] == $w['id']) ? 'selected' : '';
                            ...
                            <input type="url" name="read_online_url" class="form-control mb-3" value="<?php echo htmlspecialchars($row['read_online_url'] ?? ''); ?>" placeholder="https://...">

                            <label class="form-label mt-2">Download Link</label>
                            <input type="url" name="download_url" class="form-control mb-3" value="<?php echo htmlspecialchars($row['download_url'] ?? ''); ?>" placeholder="https://...">

                            <label class="form-label mt-2">File Size</label>
                            <input type="text" name="file_size" class="form-control mb-3" value="<?php echo htmlspecialchars($row['file_size'] ?? ''); ?>" placeholder="e.g. 2.4 MB">

                            <label class="form-label mt-2">File Type</label>
                            <input type="text" name="file_type" class="form-control mb-3" value="<?php echo htmlspecialchars($row['file_type'] ?? ''); ?>" placeholder="e.g. PDF">

                            <label class="form-label mt-2">Pages</label>
                            <input type="number" name="pages" class="form-control mb-3" value="<?php echo htmlspecialchars($row['pages'] ?? ''); ?>" placeholder="e.g. 320" min="1">
                        </div>
                    </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <select name="categories[]" id="categorySelect" class="form-select mb-2" multiple="multiple" required>
                                <?php
                                $selected_cats = [];
                                if(!empty($row['category_id'])) $selected_cats[] = $row['category_id'];
                                if(!empty($row['additional_categories'])) {
                                    $selected_cats = array_merge($selected_cats, explode(',', $row['additional_categories']));
                                }
                                
                                $cats = $conn->query("SELECT * FROM categories ORDER BY name ASC");
                                while($c = $cats->fetch_assoc()){
                                    $sel = in_array($c['id'], $selected_cats) ? 'selected' : '';
